...
def train_multiclass_step(model , dataloader , loss_fn , optimizer , device ):
  train_losses = 0.0
  train_accs = 0.0
...
def test_multiclass_step(model , dataloader , loss_fn , device) :
...
def train_multiclass(model , train_dataloader , test_dataloader , loss_fn , optimizer , epochs  , device):
  ...
  for epoch in range(epochs):
    train_losses , train_accs = train_multiclass_step(model , train_dataloader , loss_fn , optimizer , device )
    test_losses , test_accs = test_multiclass_step(model , test_dataloader , loss_fn , device )
