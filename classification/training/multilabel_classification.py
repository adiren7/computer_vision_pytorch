...
def train_step(model , dataloader , loss_fn , optimizer , confidence ,device ):
  train_losses = 0.0
  train_accs = 0.0
  ...
  for X,y in dataloader:
    ...
    y_label = (y_pred > confidence).float()
...
def test_step(model , dataloader , loss_fn , confidence,device) :
...
def train_multilabel(model , train_dataloader , test_dataloader , loss_fn , optimizer , confidence, epochs  , device):
  ...
  for epoch in range(epochs):
    train_losses , train_accs = train_step(model , train_dataloader , loss_fn , optimizer , confidence,device )
    test_losses , test_accs = test_step(model , test_dataloader , loss_fn , confidence ,device )
